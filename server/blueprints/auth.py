...
# TODO: Currently sets the default password to "hallå". Should be changed to either a random string, or a string supplied by the person who signs up the company.
    ...
    if request.method == 'POST':
        if employee.isAdmin:
            ...
            db.session.add(Employee(personID=jsonData['personID'], email=jsonData['email'], name=jsonData['name'],
                                    isAdmin=False, isBoss=False, company=employee.company, passwordHash = bcrypt.generate_password_hash("hallå").decode('utf-8'), usingDefaultPassword=True))
